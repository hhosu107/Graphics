...
int main( void )
{
	...
	Model knot = Model();
	InitDataKnot(knot, vec3(0.5f, 0.5f, 0.0f));
	...
	mat4 knot_rbt = translate(g_world_rbt, vec3(1.5f, 0.0f, 0.0f));
	g_model_rbts.push_back(knot_rbt);
	knot.SetModelRbt(&knot_rbt);
	knot.SetDirectionalLight(dir_light);

	g_models.push_back(knot);

	Model another_knot = Model();
	InitDataKnot(another_knot, vec3(0.0f, 0.5f, 0.5f));
	...
	mat4 another_knot_rbt = translate(g_world_rbt, vec3(-1.5f, 0.0f, 0.0f));
	g_model_rbts.push_back(another_knot_rbt);
	another_knot.SetModelRbt(&another_knot_rbt);
